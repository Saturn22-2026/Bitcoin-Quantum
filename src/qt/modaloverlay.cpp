      ...
      bestHeaderDate(QDateTime())
{
    ...
    m_animation.setDuration(count_milliseconds(300ms));
...
void ModalOverlay::tipUpdate(int count, const QDateTime& blockDate, double nVerificationProgress)
{
    ...
    if (blockProcessTime.size() >= 2) {
        ...
        for (int i = 1; i < blockProcessTime.size(); i++) {
            ...
            if (sample.first < (currentDate.toMSecsSinceEpoch() - count_milliseconds(500s)) || i == blockProcessTime.size() - 1) {
                progressDelta = blockProcessTime[0].second - sample.second;
                timeDelta = blockProcessTime[0].first - sample.first;
                progressPerHour = (progressDelta > 0) ? progressDelta / (double)timeDelta * count_milliseconds(1h) : 0;
